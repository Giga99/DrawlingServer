...
class Room(
    ...
    suspend fun addPlayer(clientId: String, username: String, socket: WebSocketSession): Player {
        ...
        val player = if (leftPlayers.containsKey(clientId)) {
            ...
            leftPlayer?.first?.let {
                ...
                it
            } ?: Player(username = username, socket = socket, clientId = clientId)
        } else {
            Player(username = username, socket = socket, clientId = clientId)
        }
        indexToAdd = when {
    ...
    fun removePlayer(clientId: String) {
        ...
        val announcement = Announcement(
            ...
            timestamp = System.currentTimeMillis(),
            announcementType = Announcement.AnnouncementType.TYPE_PLAYER_LEFT
        )
    ...
    suspend fun checkWordAndNotifyPlayers(message: ChatMessage): Boolean {
        if (isGuessCorrect(message)) {
            ...
            val announcement = Announcement(
                ...
                timestamp = System.currentTimeMillis(),
                announcementType = Announcement.AnnouncementType.TYPE_PLAYER_GUESSED_WORD
            )
            broadcast(gson.toJson(announcement))
            ...
            if (isRoundOver) {
                val roundOverAnnouncement = Announcement(
                    ...
                    timestamp = System.currentTimeMillis(),
                    announcementType = Announcement.AnnouncementType.TYPE_EVERYBODY_GUESSED_IT
                )
        ...
        word?.let { curWord ->
            drawingPlayer?.let { drawingPlayer ->
                val gameState = GameState(
                    drawingPlayer = drawingPlayer.username,
                    word = if (player.isDrawing || phase == Phase.SHOW_WORD) curWord else curWord.transformToUnderscores()
                )
